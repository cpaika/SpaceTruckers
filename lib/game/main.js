...
CameraGame = ig.Game.extend({
	seed: 1,
	starChance: 50, //the higher it is the more stars
	SCREEN_WIDTH: 1000,
	SCREEN_HEIGHT: 1000,
	LEVEL_WIDTH: 1000,
	LEVEL_HEIGHT:1000,
	...
	drawTerrain: function()
	{
		...
		{
			context.beginPath();
			context.moveTo(step*this.stepSize,this.terrain[step]);
			context.lineTo((step*this.stepSize)+this.stepSize,this.terrain[step+1]);
      		context.strokeStyle = "#FFFFFF";
			context.stroke(); 
			this.drawStars((step*this.stepSize), this.terrain[step]);
			step++;
		}
		this.resetRandom();
	},
	...
	generateSampleTerrain: function()
	{
		...
		{ 
			...
			var size = .01;
			sampleTerrain[i] = (PerlinNoise.noise(size*i,size*.8,0)*this.LEVEL_HEIGHT);
		}
