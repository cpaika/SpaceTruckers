...
CameraGame = ig.Game.extend({
	...
	*/
	...
	checkPlayerCollision: function()
	{
		...
		if((this.player.pos.y + this.player.size.y + 100) > this.terrain[step])
		{
			...
			for(var i = 0; i < box.length - 1;i++)
			{
				...
				var collisionResult = this.checkLineIntersect(box[i].x, box[i].y, box[i+1].x, box[i+1].y, this.player.pos.x, this.terrain[step], this.player.pos.x + this.player.size.x, this.terrain[step+1]);
	...
	drawBoundingBox: function()
	{
		...
		{
			context.fillStyle = "#FF0000";//white
			context.fillRect(box[i].x, box[i].y, 3, 3);
		}
	}
